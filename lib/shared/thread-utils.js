...
import type { GlobalAccountUserInfo, UserInfo } from '../types/user-types';
...
function createPendingThreadItem(
  ...
  const now = Date.now();
  const threadID = `pending/${user.id}`;
  ...
  const membershipPermissions = getAllThreadPermissions(
    makePermissionsBlob(permissions, null, threadID, threadTypes.PERSONAL),
    threadID,
  );
  const role = {
    id: `${threadID}/role`,
  ...
  return {
    type: 'chatThreadItem',
    threadInfo: threadInfoFromRawThreadInfo(rawThreadInfo, viewerID, {
      [user.id]: user,
    }),
    mostRecentMessageInfo: null,
    mostRecentNonLocalMessage: null,
    lastUpdatedTime: now,
    lastUpdatedTimeIncludingSidebars: now,
    sidebars: [],
